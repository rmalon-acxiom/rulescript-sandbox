...
function getBitly(url) {
	var xmlhttp = new XMLHttpRequest(),
		fullURL = 'https://api-ssl.bitly.com/v3/shorten?access_token=55d81251a8f3432a0e4ea43e3fd8a18ee9186d29&longUrl=' + url + '&format=txt';
...
function buildQueryString(id) {
	if (window.location.href.indexOf('?') > -1) {
		var baseURL = window.location.href.replace(location.search, '')
	} else {
	...
	document.getElementById("pop_in").value = fullURL;
	document.getElementById('pop_in').select();
 ...
 ***********************************************************/
var ref = new Firebase("https://luminous-torch-7743.firebaseio.com/");
...
function saveToFirebase(uuid) {
	...
	if (uuid === '') {
		...
		buildQueryString(uuidx);
	} else {
		var idRef = ruleRef.child(uuid);
		var versionRef = idRef.child("version");
		versionRef.push({
			"code": code,
			"date": Date()
		})
		buildQueryString(uuid)
	}
	version++
}
/* CLOSE POPUP
...
function exRule(editor) {
	try {
		...
		document.getElementById('exTime').innerHTML = moment.duration(tTotal).asMilliseconds().toFixed(4) + ' milliseconds. Over 1 million assemblies that would add ' + moment.duration(tTotal*1000000).humanize() + ' to a launch.';
	} catch (e) {
...
if (qs.id) {
	uuidx = qs.id
	var versionArray = []
	var idRef = ruleRef.child(qs.id) //set firbase refernce for the correct ID 
